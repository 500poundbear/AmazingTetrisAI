  ...
  private ArrayList<Individual> filterCross(ArrayList<Individual> individuals, ArrayList<Individual> sortedIndividuals) {
    ...
    for(int w=0; w < individualsLength; w++) {
      if(individuals.get(w).getFitness() <= selectionThreshold) {
        ...
        if (dice == 0) {
          // We mutate, crossing over two weak individual.
          ...
          // We perform N lambda policy iterations on the strongest individual.
          System.out.println("Lambda policy iterate");
          Individual fittestIndividual = sortedIndividuals.get(sortedIndividuals.size()-1);
          double[] rt = fittestIndividual.getWeights();
          for (int i = 0; i < N; i++)
            rt = PolicyIteration.getNext(rt);
          individuals.get(w).setWeights(rt);
        }
      }
    }
